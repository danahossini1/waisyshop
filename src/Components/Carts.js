...
const Carts = ({ name, price, groop, src1, color, url, free }) => {
    ...
    return (
        <div className='bg-white w-fit mt-5 rounded-xl relative shadow-lg' >
            {free !== 0 && <div className='absolute -left-1 top-2 bg-red-600 rounded-xl p-1 px-2 text-[11px] text-zinc-100 -rotate-[35deg]'>%free {free}</div>}
            <Link to={`category/${url}`}>
                <div className='bg-slate-200 m-1 rounded-xl flex items-center justify-center h-1/2 '>
